  ...
  def certification_status(law_firm)
    ...
    if (submissions.any?)
      ...
      if (submissions.latest.submitted)
        ...
        link_to 'Answer Follow Ups ' + "(#{submissions.latest.follow_ups.review.count})", conflicts_step_form_submission_path(submissions.latest), html_options = {class: 'btn-type-primary dashboard-certificate-button'}
      end
    else
