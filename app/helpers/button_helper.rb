    ...
    if (submissions.any?)
      ...
      if (submissions.latest.submitted)
        if submissions.latest.total_score && submissions.latest.status == 'approved'
          ...
          link_to 'Continue Certification Process', pricing_step_form_submission_path(submissions.latest), html_options = {class: 'btn btn-primary btn-lg dashboard-certificate-button'}  
        
        end
      elsif(submissions.latest.status == 'sent')
        link_to 'Begin Certification', pricing_step_form_submission_path(submissions.latest), html_options = {class: 'btn btn-primary btn-lg dashboard-certificate-button'}
      elsif(submissions.latest.status == 'started')
        link_to 'Continue Certification Process', pricing_step_form_submission_path(submissions.latest), html_options = {class: 'btn btn-primary btn-lg dashboard-certificate-button'}
