  ...
  before_action :check_new_password, only: :index
	...
	def index
  	...
  	# if current_user.is_a_standard_user?
		...
		# 	root_url
		# end
		@matter_intakes_count = (current_user&.law_firm&.matter_intakes||[]).count
		@invoices_total = (current_user.law_firm&.matter_intakes&||[]).map(&:invoices).flatten.map {|i| i.amount.to_f}&.sum
