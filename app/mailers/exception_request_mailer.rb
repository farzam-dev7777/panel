class ExceptionRequestMailer < ApplicationMailer
  ...
  def form_submission_notification_to_lxp(exception_request)
		...
		@lxpusers.each do |lxpuser| 
			mail(to: lxpuser.email, subject: "A request to engage non-panel firm request has been submitted.")
		end
	end
	...
	def form_status_notification_to_internal_lawyer(exception_request,assigned_to_id)
		...
		mail(to: @lawyer.email, subject: "A request to engage non-panel firm request has been submitted to review.")
	end
	...
	def form_status_notification_to_lob(exception_request)
		...
		mail(to: @user.email, subject: "You request to engage a non-panel firm request has been updated.")
	end

	def form_status_notification_to_lob_for_sign(exception_request)	
		@exception_request = exception_request
		@user = User.with_deactivated.find_by_id(exception_request.user_id)
		mail(to: @user.email, subject: "Your request to engage a non-panel firm status has been approved and retainer has been sent.")
	end
	...
	def form_status_notification_to_lxp(exception_request)
		...
		mail(to: @user.email, subject: "Engage non-panel first request status has been updated by the lawyer.")
	end
